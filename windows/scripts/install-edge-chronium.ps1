...
# https://raw.githubusercontent.com/matbe/PowerShell/master/Other/Get-EdgeEnterpriseMSI.ps1
function SaveEdgeChronium {
param(
  ...
  [Parameter(Mandatory = $False, HelpMessage = 'FileName which the file will be saved')]
  [ValidateNotNullOrEmpty()]
  [string]$FileName,
...
{
  $SaveFolder = "$env:TEMP"
  $SaveFile = "MicrosoftEdgeEnterpriseX64.msi"

  SaveEdgeChronium -Channel "Stable" -Platform "Windows" -Architecture "x64" -Folder "$SaveFolder" -FileName "$SaveFile"

  $MsiPath = "$SaveFolder\$SaveFile"
